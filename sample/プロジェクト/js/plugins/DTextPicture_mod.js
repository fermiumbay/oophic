...
// DTextPicture_mod.js
// ----------------------------------------------------------------------------
// 本プラグインは、DTextPicture.jsを他者(フェルミウム湾)が改変したものです。
// オリジナル版の著作権はトリアコンタン様にございます。
// 
// 【改変内容】
// ・自動生成されるピクチャ名を、日時をもとにするのではなく、描画順をもとにするよう改変しました。
// 　生成されるファイル名は以下の形式となります。
// 　「DTextPicture_(描画順(0始まり)))」
// 
// ----------------------------------------------------------------------------
// (C)2015 Triacontane
...
(function() {
    ...
    // Game_Pictureに、描画順を表すカウンタを追加
    Game_Picture.dTextPictureDrawCnt = -1;

    // Game_Pictureに、描画順を加算して返却する処理を追加
    Game_Picture.nextDrawCnt = function() {
        Game_Picture.dTextPictureDrawCnt++;
        if (Game_Picture.dTextPictureDrawCnt > 1000000) {
            Game_Picture.dTextPictureDrawCnt = 0;
        }
        return Game_Picture.dTextPictureDrawCnt;
    };
    ...
    Game_Picture.prototype.show = function(name, origin, x, y, scaleX,
        ...
        if ($gameScreen.isSettingDText() && !name) {
            arguments[0]   = "DTextPicture_" + Game_Picture.nextDrawCnt();
    ...
    Game_Picture.prototype.updateDText = function() {
        ...
        if (text !== this._dTextValue) {
            this._name = "DTextPicture_" + Game_Picture.nextDrawCnt();
        }
